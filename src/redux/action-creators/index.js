...
export const updatePassword =(datas)=> async (dispatch)=>{
  ...
  try {
    ...
    return {
      msg: "Password has been updated successfully", error:false,
    }
  } catch (error) {
    return {
      msg: error.response.data.msg, error:true,
...
export const createPatient = (patient) => async(dispatch, getState) => {
  ...
  if (patient?._id) {
    try {
      ...
      const { data:updatedPatient } = await clientAxios.put(`/patients/${patient._id}`,patient,config);
      
      const updatedPatients= patients.data.map(Patient=>Patient._id===updatedPatient._id?updatedPatient:Patient);
...
export const deletePatient =(id) => async(dispatch, getState) => {
  try {
    ...
    const updatedPatients= patients.data.filter(Patient=>Patient._id !== id)
    dispatch(setPatientsData(updatedPatients));
